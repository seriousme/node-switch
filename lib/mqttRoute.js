...
class MqttRoute {
  ...
  publish(topic, data, opts) {
    console.log("publish", topic, data, opts);
    this.mqtt.publish(
      topic,
  ...
  listen(brokerUrl, opts) {
    ...
    this.mqtt.on("message", (topic, payload) => {
      ...
      console.log(`mqttRoute resolving ${topic}`);
      const topicPaths = this.root.match(topic.split("/"));
      topicPaths.map(tp => {
        console.log(
          `mqttRoute calling ${tp.handler.name} for ${JSON.stringify({
            topic,
            data
          })}`
        );
